...
import com.xiaoai.ms.dao.IFamilyUserDao;
...
public class FamilygroupService implements IFamilygroupService {
	...
	private static Logger logger = Logger.getLogger(FamilygroupService.class);
	...
	public boolean deleteFamilygroup(Familygroup family) {
		...
		if(family.getGroupNumber()==null){
			family.setGroupNumber(groupId+10000);
		}
...
//		JSONObject array=new JSONObject();
		    ...
		    if(!XATools.isNull(roomList)){
		    	 for(Room room:roomList){
		    		 ...
		    		 for(Household household:householdList){
		    			 ...
		    			 json4.put("eaNumber", household.getEaNumber()); //家电编号 
		    			 json4.put("brand", household.getBrand()); //品牌
		    			 json4.put("brandName", household.getBrandName());//家电中文名称
		    			 ...
		    			 json4.put("stub", household.getStub()); //智能索引
		    			 /*暂时不传*/
		    			 /*if(StringUtils.isBlank(household.getStatus()+"")){
	...
	@Transactional
	@Override
	public void deleteFamilygroupDirect(Familygroup family) {
		familyDao.deleteFamilygroup(family);
		/*int groupId=family.getGroupId();
		List<FamilyUser> falist= fauserDao.selectFamilyUserBygroupid(groupId);			
		if(!XATools.isNull(falist)){    //删除家庭组用户对照表
	...
	@Transactional
	@Override
	public XiaoiResult updateFamilyAddress(Familygroup familygroup) {
		//修改单条数据，用hibernate
		
		familyDao.updateFamilyAddress(familygroup);
