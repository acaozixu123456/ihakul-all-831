...
public class RoomDao implements IRoomDao {
	...
	@Override
	public void updateRoomForRobot(final Familygroup familygroup, final String xiaoNumber) {
		//按字段更新
		hibernateTemplate.execute(new HibernateCallback<Room>() {

			@Override
			public Room doInHibernate(Session session) throws HibernateException,
					SQLException {
				String sql = "update room set robot =:empty where groupId =:groupId and robot =:robot";
				SQLQuery createSQLQuery = session.createSQLQuery(sql);
				createSQLQuery.setParameter("robot", xiaoNumber);
				createSQLQuery.setParameter("groupId", familygroup.getGroupId());
				createSQLQuery.setParameter("empty", "");
