...
import com.xiaoai.util.FastDFSClient;
...
import com.xiaoai.ms.service.IVersionService;
...
public class VersionAction extends ActionSupport {
	...
	@Value("${APK_SERVER_URL}")
	private String APK_SERVER_URL;
...
//		}
	 ...
	 * @throws  IOException
	 **/
	public  boolean fileOut() throws Exception {
		//2、创建一个FastDFS的客户端
		...
		try{
			...
			//3、执行上传处理
			path = fastDFSClient.uploadFile(buff,"apk");
			//4、拼接返回的url和ip地址，拼装成完整的url
			path= APK_SERVER_URL + path;
	...
	public String deleteVersion() throws Exception{
		...
		boolean fals = true;
		if(version!=null){
			fals=versionService.deleteVrtsion(version);
		}
