...
import com.xiaoai.ms.vo.CommonVo;
...
public class FamilygroupAction extends ActionSupport{
	...
	public String delete() throws IOException{
		...
		familyService.deleteFamilygroupDirect(fg);
		//json.put("fals", fals);
		//out.println(json.toString());
		//out.flush();
		out.println(XiaoiResult.okByJson("登录成功！"));
		return null;
	 ...
	 * @return none
	 */
	public String echartFamilyGroup() throws IOException {
		//取得所有家庭组信息
		List<Familygroup> allFamily = familyService.getAllFamily();
		List<CommonVo> familyVOS = new ArrayList<>();
		String city;
		CommonVo familyVO;
		for (Familygroup family: allFamily) {
			city = family.getCity();
			boolean flag=false;
			if(StringUtils.isNotBlank(city)){
				for (CommonVo favo:familyVOS) {
					if(city.equals(favo.getName())){
						flag=true;
						favo.setValue(favo.getValue()+1);
					}
				}
				if(!flag){
					familyVO = new CommonVo();
					familyVO.setValue(1);
					familyVO.setName(city);
		...
		/*HttpServletRequest request = MyRequest.getRequest();
		request.setAttribute("familyVOS",familyVOS);*/
		MyRequest.getResponse().print(FastJsonUtils.toJSONString(familyVOS));
		MyRequest.getResponse().flush();
		return NONE;
	}
	public int getId() {
		return id;
